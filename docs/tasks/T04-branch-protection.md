...
> ⚠️ **設定した瞬間から `git push origin main` は失敗します。** これは正常です。**全員に事前に伝えてください**（連絡帳（`Notice.md`・public 化で廃止） に1行）。以後は必ずブランチ → PR です。やり方は [T05 Git の練習](T05-git-practice.md) にあります。
...
1. **連絡帳（`Notice.md`・public 化で廃止） に1行**：「`main` への直 push を禁止しました。**今後はブランチ → PR です。** やり方は `docs/tasks/T05-git-practice.md`」
