...
**[T10](docs/tasks/T10-read-scope.md)（スコープと撤退ラインに目を通す）が完了しました。** 関口・松田の2人で[計画書 §3-8](docs/plan.md#s3-8)（スコープ）と[§12](docs/plan.md#s12)（撤退ライン）を読み合わせ、完了判定3問に回答しました：

- **1. やらないこと**：リアルタイム描画・ゲームエンジン連携は扱わない
- **2. 11/1未収束時の切り替え**：バンドル調整（①）が収束しなければ L2 に落とし、④（既知のカメラ姿勢での Visual Hull）のみでメッシュ生成に絞る
- **3. 納得できない点・不安な点**：無い

**あわせて public 化の時期を「今すぐ」に決めました。** [D-27](docs/decisions.md#d-27)・[D-40](docs/decisions.md#d-40)⑤で合意していた「完成後に public 化」から前倒しです。2人合意済み。詳細は[決定記録 D-41](docs/decisions.md#d-41)、[計画書 §16](docs/plan.md#s16) の該当行は削除済みです。

**まだやっていないこと**：リポジトリの改名（[D-27](docs/decisions.md#d-27)で「public化の直前」に行うとしていた分）と、実際のリポジトリ公開設定の変更。この2つは別途対応します。
